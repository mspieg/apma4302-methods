...
int main(int argc,char **argv) {
    ...
    PetscCall(DMDASetFieldName(da,0,"vorticity"));
    PetscCall(DMDASetFieldName(da,1,"streamfunction"));
    ...
    //output solution to vtk file for visualization
    PetscCall(DMGetGlobalVector(da,&w_initial));
    PetscCall(VecCopy(w,w_initial));
    PetscCall(PetscViewerVTKOpen(PETSC_COMM_WORLD,"biharm.vts",FILE_MODE_WRITE,&viewer));
    PetscCall(VecView(w_initial,viewer));
    ...
    PetscCall(VecStrideNorm(w_exact,0,NORM_2,&normv));
    PetscCall(VecStrideNorm(w_exact,1,NORM_2,&normu));
    PetscCall(VecAXPY(w,-1.0,w_exact));
    PetscCall(VecStrideNorm(w,0,NORM_2,&errv));
    PetscCall(VecStrideNorm(w,1,NORM_2,&erru));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
        "done on %d x %d grid ...\n"
        "  errors |v-vex|_2/|vex|_2 = %.5e, |u-uex|_2/|uex|_2 = %.5e\n",
